...
<div class="art-content-wide">
           ...
           { %>
                ...
                <% if (!String.IsNullOrEmpty(item.Columns[curCol]) && !String.IsNullOrEmpty(item.Columns[curCol].Trim()))
                   { %>
                        ...
                        <% if (Model.showToClient)
                           { %>
                            <a href='../../InvoiceReports/ClientIndex?InvoiceID=<%= Model.invoice.InvoiceID.ToString() %>&GroupID=<%= Model.invoice.BillingReportGroupID.ToString() %>'><%= Html.Encode(item.Columns[curCol])%></a>
                        <% }
                           else
                           { %>
                            <a href='../../InvoiceReports/Index?InvoiceID=<%= Model.invoice.InvoiceID.ToString() %>&GroupID=<%= Model.invoice.BillingReportGroupID.ToString() %>'><%= Html.Encode(item.Columns[curCol])%></a>
                        <% } %>
    ...
    <% if (Model.invoice.BillingGroup == 12)
    { %> 
        Please remit payment by check to the address below.<br /> Call <%=ConfigurationManager.AppSettings["billingphone"] %> if you need assistance. Thank you.
    <% }
        else
        { %>
        For your convenience we offer payment by phone with check or credit card completely free<br /> of charge. Call <%=ConfigurationManager.AppSettings["billingphone"] %> to pay by phone.  Thank you.
    <% } %>
    ...
    <% //if (Model.InvoiceType != "Finance Charge")
       //{ %>
        ...
        Visit us online at <%=ConfigurationManager.AppSettings["websiteurl"] %>
        </td>
        <td style='text-align:right; font-weight:bold; border:solid 1px black;'>
